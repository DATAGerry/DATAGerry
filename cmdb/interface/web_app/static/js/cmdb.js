...
(function (global, factory) {
  ...
  (function ($$$1, window, document, undefined) {
    ...
    function Wizard(element, options) {
      ...
      this.options = $$$1.extend(true, {}, defaults, options); // Main container element

      this.main = $$$1(element); // Navigation bar element

      this.nav = this.main.children('ul'); // Step anchor elements

      this.steps = $$$1("li > a", this.nav); // Content container

      this.container = this.main.children('div'); // Content pages

      this.pages = this.container.children('div'); // Active step index

      this.current_index = null; // Call initial method
    ...
    $$$1.extend(Wizard.prototype, {
      init: function init() {
        ...
        if (this.options.useURLhash) {
          ...
          if (hash && hash.length > 0) {
            var elm = $$$1("a[href*='" + hash + "']", this.nav);
      ...
      _setElements: function _setElements() {
        ...
        this.main.addClass('wizard-main wizard-theme-' + this.options.theme); // Set anchor elements

        this.nav.addClass('nav nav-tabs step-anchor nav-fill').children('li').addClass('nav-item').children('a').addClass('nav-link'); // nav-justified  nav-pills
        ...
        this.container.addClass('wizard-container tab-content'); // Set content pages
        ...
        if (this.options.disabledSteps && this.options.disabledSteps.length > 0) {
          $$$1.each(this.options.disabledSteps, function (i, n) {
        ...
        if (this.options.errorSteps && this.options.errorSteps.length > 0) {
          $$$1.each(this.options.errorSteps, function (i, n) {
        ...
        if (this.options.hiddenSteps && this.options.hiddenSteps.length > 0) {
          $$$1.each(this.options.hiddenSteps, function (i, n) {
      ...
      _setToolbar: function _setToolbar() {
        ...
        console.log(this.options.toolbarSettings.toolbarPosition); // Create the toolbar buttons

        var btnNext = this.options.toolbarSettings.showNextButton !== false ? $$$1('<button></button>').text(this.options.lang.next).addClass('btn btn-secondary wizard-btn-next').attr('type', 'button') : null;
        var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $$$1('<button></button>').text(this.options.lang.previous).addClass('btn btn-secondary wizard-btn-prev').attr('type', 'button') : null;
        var btnGroup = $$$1('<div></div>').addClass('btn-group mr-2 wizard-btn-group').attr('role', 'group').append(btnPrevious, btnNext); // Add extra toolbar buttons

        var btnGroupExtra = null;

        if (this.options.toolbarSettings.toolbarExtraButtons && this.options.toolbarSettings.toolbarExtraButtons.length > 0) {
          btnGroupExtra = $$$1('<div></div>').addClass('btn-group mr-2 wizard-btn-group-extra').attr('role', 'group');
          $$$1.each(this.options.toolbarSettings.toolbarExtraButtons, function (i, n) {
        ...
        switch (this.options.toolbarSettings.toolbarPosition) {
          case 'top':
            toolbarTop = $$$1('<div></div>').addClass('btn-toolbar wizard-toolbar wizard-toolbar-top justify-content-' + this.options.toolbarSettings.toolbarButtonPosition);
          ...
          case 'bottom':
            toolbarBottom = $$$1('<div></div>').addClass('btn-toolbar wizard-toolbar wizard-toolbar-bottom justify-content-' + this.options.toolbarSettings.toolbarButtonPosition);
      ...
      _setEvents: function _setEvents() {
        // Anchor click event
        var mi = this;
        $$$1(this.steps).on("click", function (e) {
          e.preventDefault();
        ...
        $$$1('.wizard-btn-next', this.main).on("click", function (e) {
          e.preventDefault();
        ...
        $$$1('.wizard-btn-prev', this.main).on("click", function (e) {
          e.preventDefault();
        ...
        if (this.options.backButtonSupport) {
          $$$1(window).on('hashchange', function (e) {
            ...
            if (window.location.hash) {
              var elm = $$$1("a[href*='" + window.location.hash + "']", mi.nav);
      ...
      _showStep: function _showStep(idx) {
      ...
      _loadStepContent: function _loadStepContent(idx) {
        ...
        if (contentURL && contentURL.length > 0 && (!elm.data('has-content') || !this.options.contentCache)) {
          ...
          var selPage = elm.length > 0 ? $$$1(elm.attr("href"), this.main) : null;
          var ajaxSettings = $$$1.extend(true, {}, {
            url: contentURL,
            type: "POST",
            data: {
              step_number: idx
      ...
      _transitPage: function _transitPage(idx) {
        var mi = this; // Get current step elements

        var curTab = this.steps.eq(this.current_index);
        var curPage = curTab.length > 0 ? $$$1(curTab.attr("href"), this.main) : null; // Get step to show elements

        var selTab = this.steps.eq(idx);
        var selPage = selTab.length > 0 ? $$$1(selTab.attr("href"), this.main) : null; // Get the direction of step navigation
      ...
      _setAnchor: function _setAnchor(idx) {
      ...
      _setButtons: function _setButtons(idx) {
        // Previous/Next Button enable/disable based on step
        if (!this.options.cycleSteps) {
          if (0 >= idx) {
            $$$1('.wizard-btn-prev', this.main).addClass("disabled");
          } else {
            $$$1('.wizard-btn-prev', this.main).removeClass("disabled");
          }

          if (this.steps.length - 1 <= idx) {
            $$$1('.wizard-btn-next', this.main).addClass("disabled");
          } else {
            $$$1('.wizard-btn-next', this.main).removeClass("disabled");
          }
        }
      ...
      _fixHeight: function _fixHeight(idx) {
        // Auto adjust height of the container
        if (this.options.autoAdjustHeight) {
          var selPage = this.steps.eq(idx).length > 0 ? $$$1(this.steps.eq(idx).attr("href"), this.main) : null;
      ...
      _setURLHash: function _setURLHash(hash) {
      ...
      theme: function theme(v) {
        ...
        this.main.removeClass('wizard-theme-' + this.options.theme);
        this.options.theme = v;
        this.main.addClass('wizard-theme-' + this.options.theme); // Trigger "themeChanged" event
      ...
      reset: function reset() {
        ...
        $$$1(".wizard-toolbar", this.main).remove();
      ...
      stepState: function stepState(stepArray, state) {
        var mi = this;
        stepArray = $$$1.isArray(stepArray) ? stepArray : [stepArray];
        var selSteps = $$$1.grep(this.steps, function (n, i) {
          return $$$1.inArray(i, stepArray) !== -1 && i !== mi.current_index;
        });

        if (selSteps && selSteps.length > 0) {
          switch (state) {
            case 'disable':
              $$$1(selSteps).parents('li').addClass('disabled');
              break;

            case 'enable':
              $$$1(selSteps).parents('li').removeClass('disabled');
              break;

            case 'hide':
              $$$1(selSteps).parents('li').addClass('hidden');
              break;

            case 'show':
              $$$1(selSteps).parents('li').removeClass('hidden');
              break;
          }
        }
      }
    ...
    $$$1.fn.wizard = function (options) {
      ...
      if (options === undefined || typeof options === 'object') {
        ...
        instance = $$$1.data(this[0], 'Wizard');

        if (options === 'destroy') {
          $$$1.data(this, 'Wizard', null);
        }
