...
class CategoryManager(FrameworkManager):
    ...
    def get(self, filter: dict, limit: int, skip: int, sort: str, order: int, *args, **kwargs) -> List[CategoryDAO]:
        return [CategoryDAO.from_data(category) for category in
                super(CategoryManager, self).get(filter=filter, limit=limit, skip=skip, sort=sort, order=order)]

    def get_one(self, public_id: Union[PublicID, int]) -> CategoryDAO:
        result = super(CategoryManager, self).get_one(public_id=public_id)
