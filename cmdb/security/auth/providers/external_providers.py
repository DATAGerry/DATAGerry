...
class LdapAuthenticationProviderConfig(AuthProviderConfig):
    ...
    def __init__(self, active: bool, default_group: int, server_config: dict, connection_config: dict, search: dict, **kwargs):
...
class LdapAuthenticationProvider(AuthenticationProvider):
    ...
    DEFAULT_PROVIDER_CONFIG = LdapAuthenticationProviderConfig(
        active=False,
        default_group=2,
        server_config={
            'host': 'localhost',
    ...
    def __init__(self, config: LdapAuthenticationProviderConfig = None):
        ...
        self.__config: LdapAuthenticationProviderConfig = config or LdapAuthenticationProvider.DEFAULT_PROVIDER_CONFIG
        super(LdapAuthenticationProvider, self).__init__(self.__config)

        self.__ldap_server = LdapAuthenticationProvider.Server(**self.__config.server_config)
    ...
    def authenticate(self, user_name: str, password: str, **kwargs) -> User:
        ...
        ldap_search_filter = self.__config.search['searchfilter'].replace("%username%", user_name)
        LOGGER.debug(f'[LdapAuthenticationProvider] Search Filter: {ldap_search_filter}')
        search_result = self.__ldap_connection.search(self.__config.search['basedn'], ldap_search_filter)
        ...
        try:
            user_instance: User = self.__user_manager.get_user_by_name(user_name=user_name)
            ...
            try:
                new_user_data = dict()
                new_user_data['public_id'] = self.__user_manager.get_new_id(User.COLLECTION)
                new_user_data['user_name'] = user_name
                new_user_data['group_id'] = self.__config.default_group
            ...
            try:
                user_id = self.__user_manager.insert_user(new_user)
            ...
            try:
                user_instance: User = self.__user_manager.get_user(public_id=user_id)
            except UserManagerGetError as umge:
