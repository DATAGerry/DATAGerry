...
*/
import { Inject, Injectable, Optional, InjectionToken, Type } from '@angular/core';
import {
  NavigationCancel,
  NavigationEnd,
  NavigationError,
...
import { Observable, of, delay, distinctUntilChanged, filter, map, switchMap, tap } from 'rxjs';
...
export class AppLoadingService {
    ...
    constructor(private router: Router,
                ...
                @Optional() @Inject(PROGRESS_ROUTER_CONFIG) config: ProgressRouterConfig) {
                    ...
                    this.router.events.pipe(
                        filter(
                            event => AppLoadingService.eventExists(event as RouterEvent, filterEvents)
                        ),
                        switchMap(
                            event => AppLoadingService.eventExists(event as RouterEvent, this.config.startEvents) ? startProgress : completeProgress)
                    ).subscribe();
    }
    ...
    public isNavigationPending: Observable<boolean> = this.router.events.pipe(
        filter(
            event => AppLoadingService.isConsideredEvent(event as RouterEvent)
        ),
        map(
            event => AppLoadingService.isNavigationStart(event as RouterEvent)
        ),
    ...
    private static isConsideredEvent(event: RouterEvent): boolean {
        return AppLoadingService.isNavigationStart(event) || AppLoadingService.isNavigationEnd(event);
    }
    ...
    static eventExists(routerEvent: RouterEvent, events: Type<RouterEvent>[]) {
        let res = false;
        events.map(
            (event: Type<RouterEvent>) => res = res || routerEvent instanceof event
        );
